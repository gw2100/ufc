...
import us.galleryw.ufc.view.UfcMenu;
...
public class TBMainView extends TestBenchTestCase {
    ...
    private CustomComponentElement getDashboardMenu() {
        return $(CustomComponentElement.class).id(UfcMenu.ID);
    }
    ...
    public int getUnreadNotificationsCount() {
        ...
        try {
            result = Integer.parseInt($(LabelElement.class).id(
                    UfcMenu.NOTIFICATIONS_BADGE_ID).getText());
...
//    }
    ...
    public int getReportsCount() {
        ...
        try {
            result = Integer.parseInt($(LabelElement.class).id(
                    UfcMenu.REPORTS_BADGE_ID).getText());
        } catch (NoSuchElementException e) {
            // Ignore
        }
        return result;
